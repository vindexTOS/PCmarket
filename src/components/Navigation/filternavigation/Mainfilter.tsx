...
function Mainfilter() {
  ...
  const { filterDropDown, setFilterDropDown } = UseProductContext()
  const style = {
    mainDiv: `w-[95%] h-[100px] bg-gray-100 rounded-[12px] flex items-center justify-end filtershaddow  gap-5`,
    dropdown: `w-[220px] h-[50px] rounded-[16px] border-[1px] flex items-center justify-end cursor-pointer  `,
    iconDiv: ` flex gap-5 `,
        ...
        {filterDropDown && (
          <div className={style.dropdownDiv}>
            {filterOptions.map((val) => (
              <div className={style.dropDownInnerDiv}>
